  ...
  return (
    <Dialog open={pasteDialog.open} onOpenChange={onPasteDialogOpenChange}>
      <DialogContent data-grid-popover=''>
        <DialogHeader>
          <DialogTitle>是否要添加更多行？</DialogTitle>
          <DialogDescription>
            需要 <strong>{pasteDialog.rowsNeeded}</strong>{' '}
            行才能粘贴剪贴板中的所有内容。
          </DialogDescription>
        </DialogHeader>
        <div className='flex flex-col gap-3 py-1'>
          <label className='flex cursor-pointer items-start gap-3'>
            ...
            <div className='flex flex-col gap-1'>
              <span className='text-sm leading-none font-medium'>创建新行</span>
              <span className='text-muted-foreground text-sm'>
                向表格添加 {pasteDialog.rowsNeeded} 行并粘贴所有数据
              </span>
            </div>
          </label>
          <label className='flex cursor-pointer items-start gap-3'>
  ...
  return (
    <input
      ...
      className={cn(
        ...
        "checked:before:bg-primary checked:before:absolute checked:before:top-1/2 checked:before:left-1/2 checked:before:size-2 checked:before:-translate-x-1/2 checked:before:-translate-y-1/2 checked:before:rounded-full checked:before:content-['']",
        'dark:bg-input/30',
        className
      )}
      {...props}

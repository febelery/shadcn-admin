...
export function UrlCell<TData>({
  ...
  const onLinkClick = React.useCallback(
    (event: React.MouseEvent<HTMLAnchorElement>) => {
      ...
      if (!href) {
        ...
        toast.error('无效的 URL', {
          description:
            'URL 包含危险的协议（javascript:、data:、vbscript: 或 file:）',
        })
        return
      }
  ...
  // 检测是否为图片 URL（通过列 ID 或文件扩展名）
  const isImageUrl =
    columnId.toLowerCase().includes('image') ||
    /\.(jpg|jpeg|png|gif|webp|svg|bmp|ico)(\?.*)?$/i.test(displayValue)
            ...
            >
              <img
                ...
                onError={(e) => {
                  ...
                  if (parent) {
                    parent.innerHTML = `<span class="text-primary decoration-primary/30 hover:decoration-primary/60 truncate underline underline-offset-2">${displayValue}</span>`
                  }
                }}
              />
            </a>
...
export function SelectCell<TData>({
  ...
  const displayLabel =
    options.find((opt: { value: string; label: string }) => opt.value === value)
      ?.label ?? value
      ...
      {displayLabels.length > 0 ? (
        <div className='flex flex-wrap items-center gap-1 overflow-hidden'>
          ...
          {hiddenBadgeCount > 0 && (
            <Badge
              variant='outline'
              className='text-muted-foreground h-5 shrink-0 px-1.5 text-xs'
            >
              +{hiddenBadgeCount}
            </Badge>
          )}
  ...
  const validateFile = React.useCallback(
    (file: File): string | null => {
      if (maxFileSize && file.size > maxFileSize) {
        return `文件大小超过 ${formatFileSize(maxFileSize)}`
      }
      if (acceptedTypes) {
  ...
  const addFiles = React.useCallback(
    async (newFiles: File[], skipUpload = false) => {
      ...
      if (maxFiles && files.length + newFiles.length > maxFiles) {
        const errorMessage = `最多允许 ${maxFiles} 个文件`
        ...
        if (firstError) {
          ...
          if (rejectedFiles.length === 1) {
            toast(firstError.reason, {
              description: `"${truncatedName}" 已被拒绝`,
            })
          } else {
            toast(firstError.reason, {
              description: `"${truncatedName}" 和其他 ${rejectedFiles.length - 1} 个文件已被拒绝`,
            })
          }
          ...
          >
            <div className='flex flex-col gap-2 p-3'>
              ...
              <div
                ...
                className='hover:bg-accent/30 focus-visible:border-ring/50 data-dragging:border-primary/30 data-invalid:border-destructive data-dragging:bg-accent/30 data-invalid:ring-destructive/20 flex cursor-pointer flex-col items-center justify-center gap-2 rounded-md border-2 border-dashed p-6 transition-colors outline-none'
                ref={dropzoneRef}
              ...
              >
                ...
                <div className='text-center text-sm'>
                  <p className='font-medium'>
                    {isDragging ? '将文件拖放到此处' : '将文件拖放到此处'}
                  </p>
                  <p className='text-muted-foreground text-xs'>或点击浏览</p>
                </div>
                <p id={descriptionId} className='text-muted-foreground text-xs'>
                  {maxFileSize
                    ? `最大大小：${formatFileSize(maxFileSize)}${maxFiles ? ` • 最多 ${maxFiles} 个文件` : ''}`
                    : maxFiles
                      ? `最多 ${maxFiles} 个文件`
                      : '选择要上传的文件'}
                </p>

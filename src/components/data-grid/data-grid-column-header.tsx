...
function getColumnVariant(variant?: CellOpts['variant']): {
  ...
  switch (variant) {
    case 'short-text':
      return { icon: BaselineIcon, label: '短文本' }
    case 'long-text':
      return { icon: TextInitialIcon, label: '长文本' }
    case 'number':
      return { icon: HashIcon, label: '数字' }
    case 'url':
      return { icon: LinkIcon, label: '链接' }
    case 'checkbox':
      return { icon: CheckSquareIcon, label: '复选框' }
    case 'select':
      return { icon: ListIcon, label: '单选' }
    case 'multi-select':
      return { icon: ListChecksIcon, label: '多选' }
    case 'date':
      return { icon: CalendarIcon, label: '日期' }
    case 'file':

    ...
    <MediaPlayerContext.Provider value={contextValue}>
      ...
      >
        ...
        <span id={descriptionId} className='sr-only'>
          {isVideo
            ? '带有自定义控件的视频播放器，支持播放、音量、跳转等功能。使用空格键播放/暂停，方向键（←/→）跳转，方向键（↑/↓）调节音量。'
            : '带有自定义控件的音频播放器，支持播放、音量、跳转等功能。使用空格键播放/暂停，Shift + 方向键（←/→）跳转，方向键（↑/↓）调节音量。'}
        </span>
        {children}
...
function MediaPlayerError(props: MediaPlayerErrorProps) {
  ...
  const errorLabel = React.useMemo(() => {
    ...
    if (!error) return '播放错误'

    const labelMap: Record<number, string> = {
      [MediaError.MEDIA_ERR_ABORTED]: '播放中断',
      [MediaError.MEDIA_ERR_NETWORK]: '连接问题',
      [MediaError.MEDIA_ERR_DECODE]: '媒体错误',
      [MediaError.MEDIA_ERR_SRC_NOT_SUPPORTED]: '不支持的格式',
    }

    return labelMap[error.code] ?? '播放错误'
  }, [label, error])
  ...
  const errorDescription = React.useMemo(() => {
    ...
    const descriptionMap: Record<number, string> = {
      [MediaError.MEDIA_ERR_ABORTED]: '媒体播放已中断',
      [MediaError.MEDIA_ERR_NETWORK]: '加载媒体时发生网络错误',
      [MediaError.MEDIA_ERR_DECODE]: '解码媒体时发生错误',
      [MediaError.MEDIA_ERR_SRC_NOT_SUPPORTED]: '不支持的媒体格式',
    }

    return descriptionMap[error.code] ?? '发生未知错误'
  }, [description, error])
...
function MediaPlayerVolumeIndicator(props: MediaPlayerVolumeIndicatorProps) {
  ...
  return (
    <VolumeIndicatorPrimitive
      role='status'
      aria-live='polite'
      aria-label={`音量 ${mediaMuted ? '静音' : `${volumePercentage}%`}`}
      ...
      <div className='fade-in-0 zoom-in-95 animate-in flex flex-col items-center gap-3 rounded-lg bg-black/30 px-6 py-4 text-white backdrop-blur-xs duration-200'>
        <div className='flex items-center gap-2'>
          ...
          <span className='text-sm font-medium tabular-nums'>
            {mediaMuted ? '静音' : `${volumePercentage}%`}
          </span>
        </div>
...
function MediaPlayerSeekForward(props: MediaPlayerSeekForwardProps) {
  ...
  return (
    <MediaPlayerTooltip
      tooltip={`前进 ${seconds} 秒`}
      shortcut={context.isVideo ? ['→'] : ['Shift →']}
    >
      <Button
        type='button'
        aria-controls={context.mediaId}
        aria-label={`前进 ${seconds} 秒`}
        data-disabled={isDisabled ? '' : undefined}
...
function MediaPlayerVolume(props: MediaPlayerVolumeProps) {
  ...
  return (
    ...
    >
      <MediaPlayerTooltip tooltip='音量' shortcut='M'>
        <Button
          id={volumeTriggerId}
          type='button'
          aria-controls={`${context.mediaId} ${sliderId}`}
          aria-label={mediaMuted ? '取消静音' : '静音'}
      ...
      <SliderPrimitive.Root
        id={sliderId}
        aria-controls={context.mediaId}
        aria-valuetext={`${Math.round(effectiveVolume * 100)}% 音量`}
        data-slider=''
...
function MediaPlayerLoop(props: MediaPlayerLoopProps) {
  ...
  return (
    <MediaPlayerTooltip
      tooltip={isLooping ? '禁用循环' : '启用循环'}
      shortcut='R'
    >
      <Button
        type='button'
        aria-controls={context.mediaId}
        aria-label={isLooping ? '禁用循环' : '启用循环'}
...
function MediaPlayerFullscreen(props: MediaPlayerFullscreenProps) {
  ...
  return (
    <MediaPlayerTooltip tooltip='全屏' shortcut='F'>
      <Button
        type='button'
...
function MediaPlayerPiP(props: MediaPlayerPiPProps) {
  ...
  return (
    <MediaPlayerTooltip tooltip='画中画' shortcut='P'>
      <Button
        type='button'
        aria-controls={context.mediaId}
...
function MediaPlayerCaptions(props: MediaPlayerCaptionsProps) {
  ...
  return (
    <MediaPlayerTooltip tooltip='字幕' shortcut='C'>
      <Button
        type='button'
        aria-controls={context.mediaId}
        aria-label={isSubtitlesActive ? '禁用字幕' : '启用字幕'}
...
function MediaPlayerDownload(props: MediaPlayerDownloadProps) {
  ...
  return (
    <MediaPlayerTooltip tooltip='下载' shortcut='D'>
      <Button
        type='button'
        aria-controls={context.mediaId}
        aria-label='下载'
        ...
        <DropdownMenuSub>
          ...
          <DropdownMenuSubContent>
            ...
            {mediaSubtitlesList.length === 0 && (
              <DropdownMenuItem disabled>无可用字幕</DropdownMenuItem>
            )}
          </DropdownMenuSubContent>
        </DropdownMenuSub>
      </DropdownMenuContent>

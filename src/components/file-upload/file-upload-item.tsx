...
export function FileUploadItem({
  ...
  const { removeFile, cardSize = 'lg' } = useFileUploadContext()
  ...
  if (view === 'card') {
    return (
      <div
        className={cn(
          'group bg-card relative aspect-square w-full overflow-hidden rounded-lg border shadow-xs',
    ...
    >
      ...
      {showDelete && (
        <button
          type='button'
          onClick={() => removeFile(item.id)}
          className='text-muted-foreground hover:bg-muted hover:text-foreground shrink-0 rounded-md p-1.5 transition-all'
          aria-label='删除文件'
        >

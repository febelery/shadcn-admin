...
export function SurfaceFillInPreview({ title }: Props) {
  ...
  if (!hasBlank) {
    return (
      <p className={cn('text-sm leading-relaxed', 'text-muted-foreground/70 border-border/60 rounded-md border border-dashed px-3 py-2.5')}>
  ...
  return (
    <p className={cn('text-foreground flex flex-wrap items-baseline gap-x-0.5', 'text-sm leading-relaxed font-normal placeholder:text-muted-foreground/50')}>
      {segments.map((seg, i) =>
        seg.kind === 'text' ? (

  ...
  return (
    <div className='bg-muted/50 rounded-md border p-2'>
      <div className='grid grid-cols-4 gap-1 text-[9px]'>
        <div />
        {['A', 'B', 'C'].map((c) => (
          <div key={c} className='text-muted-foreground text-center font-medium'>
            {c}
          </div>
        ))}
        {['行1', '行2'].map((row) => (
...
function LikertPreview() {
  ...
  return (
    <div className='bg-muted/50 overflow-hidden rounded-md border text-[9px]'>
      <div className='grid grid-cols-6 gap-px border-b bg-muted/80 p-1'>
        <div />
        {cols.map((c) => (
        ...
        >
          ...
          {cols.map((c, i) => (
            <div key={c} className='flex justify-center'>
              <div
                className={cn(
                  'size-2 rounded-full border border-muted-foreground/35',
                  i === 2 &&
                    'border-primary/60 bg-primary/25 ring-1 ring-primary/20'
                )}
              />
            </div>
          ))}

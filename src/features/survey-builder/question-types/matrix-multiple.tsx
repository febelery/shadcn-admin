 ...
 * 3. 导出定义
 */
export const matrixMultipleType: QuestionTypeDefinition = {
  ...
  create: () => ({
    ...
    required: false,
    config: {
      rows: [{ id: crypto.randomUUID(), label: '维度 1' }],
      columns: [
          ...
          <div key={i} className='flex items-center gap-1.5'>
            ...
            <div className='flex gap-1.5'>
              {cols.slice(0, 3).map((_, j) => (
                <div
                  key={j}
                  className='border-muted-foreground/30 h-3 w-3 rounded-sm border'
                />
              ))}
            </div>
          </div>
          ...
          <div className='space-y-1.5'>
            {rows.map((row) => (
              <div key={row.id} className='group flex items-center gap-2'>
                <Input
                  className='bg-muted/20 hover:bg-muted/40 h-7 flex-1 border-transparent text-xs shadow-none transition-colors'
                  value={row.label}

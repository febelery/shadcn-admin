 ...
 * 3. 导出定义
 */
export const fileUploadType: QuestionTypeDefinition = {
  type: 'file_upload',
  meta: {
    label: '文件上传',
  ...
  configPanel: function ConfigPanel({ node }: { node: QuestionNode }) {
    ...
    const toggleType = (t: string) => {
      ...
      const next = current.includes(t)
        ...
        : [...current, t]
      updateNodeConfig(node.id, { acceptTypes: next })
    }

    return (
      <div className='p-3 space-y-5 font-sans'>
        {/* 按钮文本 */}
        <div className='space-y-2'>
          ...
          <Input
            className='bg-muted/20 hover:bg-muted/40 h-8 border-transparent text-xs transition-colors shadow-none'
          ...
          <div className='flex flex-wrap gap-1.5'>
            {mimeOptions.map((opt) => (
              <Badge
                key={opt.value}
                variant={
                  config.acceptTypes?.includes(opt.value) ? 'default' : 'outline'
                }
                className='h-5 cursor-pointer px-1.5 text-[9px] transition-all shadow-none'
        ...
        <div className='border-border/40 grid grid-cols-2 gap-3 border-t pt-3'>
          <div className='space-y-1.5'>
            ...
            <Input
              ...
              onChange={(e) =>
                updateNodeConfig(node.id, { maxCount: +e.target.value })
              }
            />
          </div>
          <div className='space-y-1.5'>
            ...
            <Input
              ...
              onChange={(e) =>
                updateNodeConfig(node.id, { maxSize: +e.target.value })
              }
            />
          </div>
        </div>

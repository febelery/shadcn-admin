 ...
 * 3. 题型组件导出
 */
export const npsType: QuestionTypeDefinition = {
  type: 'nps',
  meta: {
    label: 'NPS',
        ...
        <div className='space-y-3'>
          <div className='flex items-center justify-between'>
            ...
            <Switch
              ...
              onCheckedChange={(v) =>
                updateNodeConfig(node.id, { showLabels: v })
              }
            />
          </div>
        ...
        <div className='border-border/40 space-y-3 border-t pt-3'>
          ...
          <div className='space-y-1.5'>
            <Input
              ...
              placeholder='批评者 (0–6)'
              onChange={(e) =>
                updateNodeConfig(node.id, { detractorLabel: e.target.value })
              }
            />
            <Input
              className='bg-muted/20 hover:bg-muted/40 h-7 border-transparent text-xs shadow-none transition-colors'
              ...
              placeholder='中立者 (7–8)'
              onChange={(e) =>
                updateNodeConfig(node.id, { passiveLabel: e.target.value })
              }
            />
            <Input
              className='bg-muted/20 hover:bg-muted/40 h-7 border-transparent text-xs shadow-none transition-colors'
              ...
              onChange={(e) =>
                updateNodeConfig(node.id, { promoterLabel: e.target.value })
              }
            />
          </div>
        </div>
  ...
  editor: function Editor({ node }: { node: QuestionNode }) {
    ...
    const getColorClass = (v: number) => {
      const threshold6 = scaleStart === 0 ? 6 : 6
      const threshold8 = scaleStart === 0 ? 8 : 8

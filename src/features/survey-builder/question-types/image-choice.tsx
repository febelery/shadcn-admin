 ...
 * 3. 题型定义
 */
export const imageChoiceType: QuestionTypeDefinition = {
  type: 'image_choice',
  meta: {
    label: '图片选择',
            ...
            >
              ...
              <div className='space-y-1.5'>
                <input
                  ...
                  value={opt.label}
                  placeholder={`选项标题...`}
                  onChange={(e) => updateField(opt.id, 'label', e.target.value)}
                />
                <div className='text-muted-foreground/50 flex items-center gap-2'>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateField(opt.id, 'image', e.target.value)
                    }
                  />
                </div>
              </div>
  ...
  editor: function Editor({ node }: { node: QuestionNode }) {
    const options = (node.config.options ?? []) as ChoiceOption[]
    const { addOption, removeOption, updateLabel, updateImage } =

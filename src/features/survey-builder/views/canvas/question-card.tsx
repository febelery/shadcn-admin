 ...
 */
const QuestionCardContent = React.memo(
  function QuestionCardContent({
    ...
    const node = useSchemaStore((s) => s.nodes.find((n) => n.id === id))
    ...
    return (
      <TooltipProvider>
        ...
        >
          ...
          <div className='px-8 py-8 pl-8 transition-all lg:pr-24'>
            {/* 顶部元信息行：题号、类型标签、必填标记 */}
...
const DisplayTitle = React.memo(
  ...
  (prev, next) =>
    prev.node.id === next.node.id && prev.node.title === next.node.title
)

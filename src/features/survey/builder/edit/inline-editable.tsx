...
export function InlineEditable({
  ...
  return (
    <div
      ...
      className={cn(
        'outline-none',
        'empty:before:text-muted-foreground/50 empty:before:pointer-events-none empty:before:content-[attr(data-placeholder)]',
        multiline &&
          !compact &&
          'min-h-[1.5em] wrap-break-word whitespace-pre-wrap',
        !multiline && !compact && 'min-h-[1.25em] min-w-0 wrap-break-word',
        compact && !focused && 'max-w-full min-w-0 truncate whitespace-nowrap',
        compact &&
          focused &&
          'max-w-full min-w-0 overflow-visible break-all whitespace-normal',
        className
      )}
      onInput={syncFromDom}
      onBlur={() => {

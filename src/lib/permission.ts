...
function matchSingle(held: string, required: string): boolean {
  ...
  // resource 级通配：held = "user:*", required = "user:access"
  if (held.endsWith(':*')) {
    const heldResource = held.slice(0, -2) // "user"
 ...
 * @param required - 需要检查的权限字符串，如 "user:access"
 * @returns 是否具备该权限
 *
 * @example
 * ```ts
 * hasPermission(['*'], 'user:access')           // true（超管）
 * hasPermission(['user:*'], 'user:access')      // true（user 模块全权限）
 * hasPermission(['user:access'], 'user:access') // true（精确命中）
 * hasPermission(['task:access'], 'user:access') // false
 * ```
 */

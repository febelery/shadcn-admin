...
const AVAILABLE_PERMISSIONS = [
  ...
  { key: 'setting:access', label: '系统设置', group: '系统' },
  { key: 'component-demo:access', label: '组件演示', group: '其他' },
]
...
export const permissionHandlers = [
  // 获取所有角色
  http.get('/api/role', async () => {
    await sleep(300)
    return HttpResponse.json(mockRoles)
  }),

  // 获取单个角色
  http.get('/api/role/:id', async ({ params }) => {
    await sleep(200)
  ...
  // 更新角色
  http.put('/api/role/:id', async ({ params, request }) => {
    await sleep(300)
  ...
  // 删除角色
  http.delete('/api/role/:id', async ({ params }) => {
    await sleep(200)
    const index = mockRoles.findIndex((r) => r.id === params.id)
    if (index === -1) {
      return HttpResponse.json(
  ...
  // 获取所有可用权限列表
  http.get('/api/permission/available', async () => {
    await sleep(100)
